...
public class PlayerController : MonoBehaviour
{
    ...
    [Header("\"Watch For Coins\" feature config")]
    public bool disableRewardedAds = false;
    public int rewardedCoins = 50;
    public int gamesPerRewardedAd = 1;
    ...
    void Update()
    {
        if (Input.GetMouseButtonDown(0) && enableJump)
        {
    ...
    void GameOver()
    {
        ...
        gameCountForRewardedAds++;
        StartCoroutine(CRShowWatchAdForCoins(1f));     
    }
    ...
    IEnumerator CRShowWatchAdForCoins(float delay = 0f)
    {
        ...
        if (gameCountForRewardedAds >= gamesPerRewardedAd)
        {
            bool isAdReady = AdsManager.Instance != null && AdsManager.Instance.IsAdReady();

            #if UNITY_EDITOR
            isAdReady = true;   // for testing
            #endif

            if (!disableRewardedAds && isAdReady)
            {
                uiController.ShowWatchForCoinsBtn();
                gameCountForRewardedAds = 0;
            }
        }
    }

    public void WatchRewardedVideo(float delay = 0f)
    {
        StartCoroutine(CRWatchAdForCoins(delay));
    }

    IEnumerator CRWatchAdForCoins(float delay = 0f)
    {
        yield return new WaitForSeconds(delay);

        uiController.HideWatchForCoinsBtn();
    ...
    void RewardCoins()
    {
        CoinManager.Instance.AddCoins(rewardedCoins);

        // Show message to inform user of their reward!
        uiController.ShowToastMsg("you have earned " + rewardedCoins + " coins!"); // this toast msg will auto hide after 5s
    }
    ...
    IEnumerator ButtonClick(string buttonName)
    {
        if (enableJump)
        {
            ...
            {
                uiController.HideColoredButtons();
            }
